...
const Sidebar = () => {
  ...
  const handleTopLinkClick = () => {
    setActiveBottomLink(null);  // Reset bottom links when clicking top links
    handleCloseSideBar();       // Close sidebar if needed
  };
      ...
      <NavLink
        ...
        className={({ isActive }) =>
          `flex items-center gap-5 pl-4 pt-3 pb-2.5 rounded-lg text-md m-2 
           ${link.name === 'logout' 
              ? 'text-red-600 hover:text-red-700'  // Red text only, no bg
              : activeBottomLink === link.name  // Only the clicked link is active
