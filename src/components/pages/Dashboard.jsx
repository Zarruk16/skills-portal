  ...
  return (
    <div className='mt-[60px] md:mt-30 lg:mt-5 max-w-7xl mx-auto p-4 space-y-8'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-12 gap-6'>
        ...
        <div className='lg:col-span-4 bg-white dark:text-gray-200 dark:bg-secondary-dark-bg p-6 rounded-xl shadow-md'>
          <h2 className='text-xl font-semibold mb-4'>Recent Applications</h2>
          <ul className='divide-y divide-gray-300 w-full h-full mt-6'>
            {recentApplications.map((applicant, index) => (
              <li
                key={index}
                className='flex items-center justify-between p-3 dark:bg-gray-700 mb-3'
                style={{ lineHeight: '1.8' }}
              >
                <span className='font-medium'>{applicant.name}</span>
                <span
                  className={`px-4 py-3 text-sm rounded-lg ${applicant.color}`}
                >
                  {applicant.role}
                </span>
              </li>
